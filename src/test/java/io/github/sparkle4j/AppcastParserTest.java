...
class AppcastParserTest {
    ...
    @Test
    @DisplayName("parses valid appcast and returns items sorted by version descending")
    void parsesValidAppcast() throws Exception {
        ...
        for (int i = 0; i < items.size() - 1; i++) {
            assertTrue(
                    VersionComparator.isNewer(items.get(i).version(), items.get(i + 1).version())
                            || items.get(i).version().equals(items.get(i + 1).version()));
        }
    }

    @Test
    @DisplayName("filters to current OS only")
    ...
    @Test
    @DisplayName("rejects non-HTTPS enclosure URLs")
    ...
    @Test
    @DisplayName("parses critical update flag")
    ...
    @Test
    @DisplayName("returns empty list for malformed XML")
    void malformedXml() {
        assertThrows(Exception.class, () -> parser.parse("not xml at all"));
    }

    @Test
    @DisplayName("returns empty list when no enclosure matches current OS")
    void noMatchingOs() throws Exception {
        var otherOs = "windows".equals(AppcastParser.currentOs()) ? "macos" : "windows";
        var xml =
                """
            <?xml version="1.0" encoding="UTF-8"?>
            <rss version="2.0" xmlns:sparkle="http://www.andymatuschak.org/xml-namespaces/sparkle">
    ...
    @Test
    @DisplayName("enclosure version overrides item version")
    ...
    @Test
    @DisplayName("filters by minimumJavaVersion")
    ...
    @Test
    @DisplayName("uses inline description when no release notes URL")
    ...
    @Test
    @DisplayName("shortVersionString falls back to version when absent")
    ...
    @Test
    @DisplayName("default installer type is inno")
