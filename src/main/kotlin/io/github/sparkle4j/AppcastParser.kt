...
internal class AppcastParser {
    ...
    fun parse(xml: String): List<UpdateItem> {
        ...
        for (i in 0 until itemNodes.length) {
            ...
            parseItem(item, currentOs, currentJvmMajor)?.let { result.add(it) }
        }
    ...
    private fun parseItem(item: Element, currentOs: String, currentJvmMajor: Int): UpdateItem? {
        ...
        return findMatchingEnclosure(item, currentOs, itemVersion, itemShortVersion, releaseNotesUrl, inlineDescription, title, pubDate, minJava, isCritical)
    }

    @Suppress("LongParameterList")
    private fun findMatchingEnclosure(
        item: Element,
        currentOs: String,
        itemVersion: String?,
        ...
        for (j in 0 until enclosures.length) {
            ...
            val encItem = parseEnclosure(enc, currentOs, itemVersion, itemShortVersion, releaseNotesUrl, inlineDescription, title, pubDate, minJava, isCritical)
            if (encItem != null) return encItem
        }
        return null
    }
    ...
    private fun parseEnclosure(
        ...
        if (os != currentOs) return null

        val url = enc.getAttribute("url").takeIf { !it.isNullOrBlank() } ?: return null
    ...
    private fun Element.sparkleElement(localName: String): String? = getElementsByTagNameNS(SPARKLE_NS, localName)
        .takeIf { it.length > 0 }
        ?.item(0)?.textContent?.trim()
        ?.takeIf { it.isNotBlank() }

    private fun Element.firstElementText(tagName: String): String? = getElementsByTagName(tagName)
        .takeIf { it.length > 0 }
        ?.item(0)?.textContent?.trim()
        ?.takeIf { it.isNotBlank() }
