 ...
 *   <li>Resume support via HTTP Range header (retries partial download)</li>
 * </ul>
 */
public class UpdateDownloader {
    ...
    /**
     * Downloads the given URL to a temp file named after the URL's last path segment.
    ...
    public Path download(String url, long totalBytes, BiConsumer<Long, Long> onProgress)
        ...
        long resumeAt = switch (response.statusCode()) {
            case 206 -> existingBytes;
            case 200 -> {
                Files.deleteIfExists(tempFile);
                yield 0L;
            }
            default -> throw new IOException("HTTP " + response.statusCode() + " downloading " + url);
        };
    ...
    private void streamToFile(InputStream input, Path tempFile, long resumeAt, long totalBytes,
                              BiConsumer<Long, Long> onProgress) throws IOException, InterruptedException {
        var openOptions = resumeAt > 0
            ? new StandardOpenOption[]{StandardOpenOption.APPEND}
            : new StandardOpenOption[]{StandardOpenOption.CREATE, StandardOpenOption.TRUNCATE_EXISTING};

        try (input; OutputStream output = Files.newOutputStream(tempFile, openOptions)) {
