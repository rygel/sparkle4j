 ...
 * @param shortVersionString Human-readable version for display (from {@code sparkle:shortVersionString}),
 *                           falls back to {@code version} when absent
 ...
 * @param inlineDescription Inline release notes from {@code <description>} (fallback when releaseNotesUrl is absent)
 * @param minimumJavaVersion Minimum Java major version required (custom {@code sparkle:minimumJavaVersion} element)
 * @param isCriticalUpdate  When true the "Skip This Version" button is hidden (set by {@code <sparkle:criticalUpdate/>})
